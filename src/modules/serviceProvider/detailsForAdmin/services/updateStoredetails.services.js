const storeDetails = require("../../storeDetails/storeDetail.model");
const ServiceProviderModel = require("../../serviceProviderCreation/serviceProvider.model");
const mongoose = require("mongoose");

const adminupdateStoreDetails = async ({id,data} ) => {
    ...
    try {
        ...
        const bankDetails = await storeDetails.findById(id);

        if (!bankDetails) {
            return { data: "Bank Details Not Found", status: false, code: 400 };
        } else {
            const updateResult = await storeDetails.findOneAndUpdate(
                bankSearch,
                { ...data },
                { new: true }
            );
...
module.exports = adminupdateStoreDetails;
